...
SP = "/tmp/claude-0/-root-egosmith/6b57c32e-2fab-4780-bf21-8479371681d9/scratchpad"
_src = open(f"{SP}/ablation_sampling_detail.py").read().split("clips = pick_clips(10)")[0]
ns = {}
...
_DS = os.environ.get("EGO_DATASET", "egocentric100k")
_B = f"gs://foundational-research/hoi-dataset/egosmith_filtered/{_DS}"
GCS_FILT = f"{_B}/filter_run/_shards"
GCS_FR = f"{_B}/frames"
GCS_ANN = f"{_B}/filter_run/annotations_v4/_shards"
OUT = os.environ.get("EGO_LABEL_OUT", f"/root/egosmith_annotations/{'ego' if _DS=='egocentric100k' else _DS}")
PX = int(os.environ.get("EGO_PX", "1024"))
DETAIL = os.environ.get("EGO_DETAIL", "high")
TARGET_FPS = float(os.environ.get("EGO_FPS", "3.0"))
FMIN = int(os.environ.get("EGO_FMIN", "8"))
FMAX = int(os.environ.get("EGO_FMAX", "40"))
SEED_FROM_EXTRA = os.environ.get("EGO_SEED_FROM_EXTRA", "0") == "1"
...
def annotate_clip(sfx, rec_json):
    ...
    try:
        ...
        if SEED_FROM_EXTRA:
            ex = (d.get("extra") or {})
            seeds = ex.get("annotations") or []
            task = ex.get("task") or ""
            if seeds or task:
                fps_src = float(d.get("fps") or 30.0)
                rng = (ex.get("segment_frame_range") or [0, 10**9])
                lines = []
                for s in seeds:
                    a, b = int(s.get("start_idx", 0)), int(s.get("end_idx", 0))
                    if b < rng[0] or a > rng[1]:
                        continue
                    t0 = max(0.0, (a - rng[0]) / fps_src)
                    t1 = max(t0, (min(b, rng[1]) - rng[0]) / fps_src)
                    lines.append(f"  {t0:.1f}-{t1:.1f}s: {s.get('text','')}")
                seed_txt = ("CAPTURE-SYSTEM PRIOR (ground-truth activity labels recorded "
                            "with this clip; trust them for WHAT happens, refine WHERE/HOW "
                            "from the frames):\n")
                if task:
                    seed_txt += f"  episode task: {task}\n"
                if lines:
                    seed_txt += "\n".join(lines)
                content.append({"type": "text", "text": seed_txt})
        last = None
        ...
        row = {"clip_id": clip_id, "dataset": _DS, "shard": sfx,
