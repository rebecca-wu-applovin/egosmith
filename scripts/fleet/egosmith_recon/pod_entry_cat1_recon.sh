#!/usr/bin/env bash
# pod_entry_cat1_recon.sh — Phase C for Cat-1 video-source datasets. Variant of
# pod_entry_egocentric_recon.sh (unchanged, still the 100K/10K entry) with:
#   * est_focal.txt written only when extra.pinhole_focal exists (fisheye-undistorted
#     assembly101/hd_epic sub-clips); pinhole sources have no prior focal;
#   * USE_ANYCALIB=1 (default): batch_infer --use_anycalib estimates the focal in
#     detect_track for clips without est_focal.txt (GT-validated: 9.0%/3.6% median
#     focal err on taco/hot3d vs 30.3%/15.6% for the W/2 guess). Requires the v2
#     image (anycalib installed); weights staged from GCS (offline-proof).
#
...
echo "[cat1_recon] START shard=${SHARD:-${JOB_COMPLETION_INDEX:-?}} @ $(date +%F_%H:%M:%S)"
...
case "$SHARD" in ''|*[!0-9]*) echo "[cat1_recon] FATAL: SHARD not numeric ('$SHARD')"; exit 1;; esac
...
log(){ echo "[cat1_recon][shard $SHARD/$NSHARDS] $*"; }
...
trap 'echo "[cat1_recon] EXIT disk state:"; df -h /scratch 2>/dev/null | tail -1; du -sh "$W"/* 2>/dev/null | sort -rh | head -5' EXIT
