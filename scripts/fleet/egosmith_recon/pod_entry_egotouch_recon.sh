#!/usr/bin/env bash
# pod_entry_egotouch_recon.sh — Phase C for EgoTouch (W7 seeded run). Variant of
# pod_entry_cat1_recon.sh (unchanged) with:
#   * HaMeR-box SEEDING between tar pull and batch_infer: seed_tracks_egotouch.py
#     synthesizes tracks_0_N/{model_boxes,model_tracks}.npy from the dataset's own
#     hamer/wilor camera-frame joints, so detect_track --resume skips the glove-blind
#     YOLO stage (W7 pilot: 10/19 presence-gated keeps seeded vs 0/19 unseeded);
#   * USE_ANYCALIB=0 default (pilot regime: no est_focal -> W/2=228 fallback focal);
#   * default GCS prefixes -> egosmith_filtered/egotouch + egosmith_recon/egotouch.
#
...
echo "[egotouch_recon] START shard=${SHARD:-${JOB_COMPLETION_INDEX:-?}} @ $(date +%F_%H:%M:%S)"
...
case "$SHARD" in ''|*[!0-9]*) echo "[egotouch_recon] FATAL: SHARD not numeric ('$SHARD')"; exit 1;; esac
...
FRAMES_GCS="${FRAMES_GCS:-gs://foundational-research/hoi-dataset/egosmith_filtered/egotouch/frames}"
MANIFESTS_GCS="${MANIFESTS_GCS:-gs://foundational-research/hoi-dataset/egosmith_filtered/egotouch/phaseB/_shards}"
OUT_GCS="${OUT_GCS:-gs://foundational-research/hoi-dataset/egosmith_recon/egotouch/recon/outputs}"
LOGS_GCS="${LOGS_GCS:-gs://foundational-research/hoi-dataset/egosmith_recon/_logs/egotouch}"
sfx=$(printf '%05d' "$SHARD")
log(){ echo "[egotouch_recon][shard $SHARD/$NSHARDS] $*"; }
...
USE_ANYCALIB="${USE_ANYCALIB:-0}"
ANYCALIB_FLAG=""
if [ "$USE_ANYCALIB" = "1" ]; then
  ANYCALIB_FLAG="--use_anycalib"
...
trap 'echo "[egotouch_recon] EXIT disk state:"; df -h /scratch 2>/dev/null | tail -1; du -sh "$W"/* 2>/dev/null | sort -rh | head -5' EXIT
...
for line in open(f"{W}/manifest.raw.jsonl"):
    ...
    focal = d["extra"].get("pinhole_focal")   # fisheye-undistorted sources only
    if focal is not None:
        open(f"{seq}/est_focal.txt", "w").write(f"{float(focal):.6f}\n")
...
# ── 3.5 HaMeR-box seeding: synthesize detect_track outputs BEFORE batch_infer ──
...
# 0 seeded on a FRESH shard = mirror/json outage -> retry. On a resume tail the
# leftovers may legitimately be the no-json episodes — proceed (YOLO fallback).
if [ "$SEEDED" -eq 0 ] && [ "$(cat "$W/skipped_count" 2>/dev/null || echo 0)" -eq 0 ]; then
  log "FATAL: 0 clips seeded on fresh shard (mirror/json outage?) — retrying"; exit 1
fi
