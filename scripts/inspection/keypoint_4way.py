...
clips=[c.strip() for c in open(f"{SC}/egodex_4way_clips.txt") if c.strip()]
...
for clip in clips:
    ...
    for m,root in (("recon","/root/egodex_4way/recon"),("anycalib","/root/egodex_4way/recon_anycalib")):
        seq=f"{root}/{clip}"
    ...
    ef=f"/root/egodex_4way/egoforce/{clip}.npz"
