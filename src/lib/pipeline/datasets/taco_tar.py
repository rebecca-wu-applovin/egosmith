"""Dataset adapter for TACO sequence-per-tar shards (GT ingestion).

Tars are produced by ``scripts/build/generate_taco_world_res.py``:
one ``TACO_<tool>_<action>_<object>_<date>_<id>.tar`` per sequence containing
``<clip_id>_f%05d.image.jpg`` frames, with GT-derived stage outputs in the
matching ``seq_folder`` (world_space_res.pth, SLAM npz, infiller marker).
"""
...
_TAR_NAME_RE = re.compile(r"^(TACO_(?P<tool>[A-Za-z0-9-]+)_(?P<action>[A-Za-z0-9-]+)_(?P<object>[A-Za-z0-9-]+)_(?P<seq>\d{8}_\d+))\.tar$")
_IMAGE_MEMBER_RE = re.compile(r"^(TACO_.+)_f(\d+)\.image\.jpg$", re.IGNORECASE)


def _parse_tar_identity(tar_path: Path) -> tuple[str, str, str, str, str]:
    match = _TAR_NAME_RE.match(tar_path.name)
    if match is None:
        raise ValueError(
            "TACO tar filename must match `TACO_<tool>_<action>_<object>_<date>_<id>.tar`, "
            f"got: {tar_path.name}"
        )
    clip_id = match.group(1)
    return clip_id, match.group("tool"), match.group("action"), match.group("object"), match.group("seq")
...
class TACOTarDatasetAdapter(BaseDatasetAdapter):
    name = "taco_tar"
    ...
    def build_descriptors(
        ...
        *,
        ...
        for tar_path in sorted(tar_root.glob("*.tar")):
            clip_id, tool, action, obj, seq_name = _parse_tar_identity(tar_path)
            ...
            descriptors.append(
                ClipDescriptor.from_tar_shard(
                    ...
                    extra={
                        "adapter": self.name,
                        "dataset_name": dataset_cfg.get("source_id") or "taco",
                        "tool": tool,
                        "action": action,
                        "object": obj,
                        "taco_seq_name": seq_name,
                    },
                )
            )
        return descriptors
